...
(function($){
        ...
        $('.ui-pagination-control')
                .live('disable', function(){
                        // Despite the styling, IE wasn't rendering the opacity from CSS correctly
                        $(this).addClass('ui-state-disabled').css('opacity', .35);
                })
                ...
                .live('click', function(){
                        ...
                        switch (el.attr('ref')){
                                ...
                                default: data.current = el.attr('ref'); break;
                        }
        ...
        $.pagination = {
                defaults: {
                        ...
                        templates: {
                                first: '<a class="ui-pagination-control ui-pagination-decrement ui-pagination-first" href="#" ref="first"></a>',
                                last: '<a class="ui-pagination-control ui-pagination-increment ui-pagination-last" href="#" ref="last"></a>',
                                next: '<a class="ui-pagination-control ui-pagination-increment ui-pagination-next" href="#" ref="next"></a>',
                                previous: '<a class="ui-pagination-control ui-pagination-decrement ui-pagination-last" href="#" ref="previous"></a>',
                                number: '<a class="ui-pagination-control ui-pagination-digit" href="#" ref="#"></a>',
        ...
        $.fn.pagination = function(group, options, callback){
                ...
                if (typeof options == 'function' && !callback){ callback = options; options = {} }
               
                switch (group){
                                // key, group, options
                        case 'renumber':
                                callback = callback||this.attr('rel');
                                ...
                                // key, group, callback
                        case 'callback': $.extend($.pagination.groups[options], {callback: callback}); break;
                       
                                // key, group, options
                        case 'force-change':
                                $.extend($.pagination.groups[options], callback);
                                paginationCleanup($.pagination.groups[options], options);
                                break;
        ...
        function createPagination(group, options, callback){
                ...
                this.each(function(){
                        ...
                        el.addClass('ui-pagination').attr('rel', group);
                       
                        // Build the pagination from left to right
                        var f = $(options.templates.first).text(options.text.first).appendTo(el),
                                p = $(options.templates.previous).text(options.text.previous).appendTo(el),
                                n = $(options.templates.next).text(options.text.next),
                                l = $(options.templates.last).text(options.text.last);
        ...
        function createInitialDigits(data, group){
                ...
                $.each(data.frontDigits, function(k){
                        $.each(this, function(){ if (pages) this.attr('ref', current).text(current+1).show(); else this.hide() })
                        current++;
                        if (pages) pages--;
                })
                ...
                $.each(data.rearDigits, function(k){
                        $.each(this, function(){ if (pages) this.attr('ref', rearPages-1).text(rearPages).show(); else this.hide() })
                        rearPages--;
                        if (pages) pages--;
                })
        ...
        function paginationCleanup(data, group){
                ...
                var current = data.current*1,
                        ...
                        lowBounding = data.frontDigits.length ? data.frontDigits.last()[0].attr('ref')*1 : -1,
                        highBounding = data.rearDigits.length ? data.rearDigits.last()[0].attr('ref')*1 : max+1,
                        lowDisplay = data.digits[0][0].attr('ref')*1||0;
                        highDisplay = data.digits[last][0].attr('ref')*1||max;
                ...
                // Rebuild the indexs
                lowDisplay = data.digits[0][0].attr('ref')*1;
                highDisplay = data.digits[last][0].attr('ref')*1;
                ...
                $.each(data.allDigits, function(){
                        var mode = this[0].attr('ref') == data.current ? 'disable' : 'enable';
        ...
        function renumberDigits(data, start){
                $.each(data.digits, function(){
                        $.each(this, function(){ this.attr('ref', start).text(start+1) })
                        start++;
                })
        }
